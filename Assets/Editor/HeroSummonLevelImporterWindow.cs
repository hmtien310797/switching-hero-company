...
namespace Immortal_Switch.Editor.GameData
{
    public sealed class HeroSummonLevelImporterWindow : EditorWindow
    {
        ...
        private const string GoogleSheetCsvUrl =
            "https://docs.google.com/spreadsheets/d/e/2PACX-1vQq5Rq5h3ZiaDfG8U6-Q3hytEOHs3DqRgBETG7qcE2LjQZAhwR971MjEZqgc6wmsb_1Ey1mPK9-R13S/pub?gid=1720872520&single=true&output=csv";

        private const string ConfigAssetPath =
            "Assets/Immortal Switch/Addressable/SummonData/HeroSummonConfig.asset";
        ...
        {
            var window = GetWindow<HeroSummonLevelImporterWindow>();
            window.titleContent = new GUIContent("Hero Summon Importer");
            window.minSize = new Vector2(680f, 540f);
        }

        private void OnGUI()
        {
            EditorGUILayout.LabelField("Hero Summon Level Importer", EditorStyles.boldLabel);
        ...
        private void DownloadGoogleSheetAndApply()
        {
            ...
            isDownloading = true;
            message = "Đang tải Hero Summon CSV từ Google Sheet...";
        ...
        private void ParseAndApply(string csvText)
        {
            try
            {
                ScriptableObject config =
                    AssetDatabase.LoadAssetAtPath<ScriptableObject>(ConfigAssetPath);

                if (config == null)
                {
                    throw new InvalidOperationException(
                        $"Không tìm thấy ScriptableObject tại đường dẫn: {ConfigAssetPath}");
                }
                ...
                Undo.RecordObject(config, "Import Hero Summon Levels");
                ...
                message = $"Imported {levelCount} hero summon levels and {rewardCount} rewards.";
            }
            catch (Exception ex)
            {
                Debug.LogException(ex);
        ...
        private static List<Row> ParseRows(string input)
        {
            ...
            for (int i = headerIndex + 1; i < lines.Length; i++)
            {
                ...
                var row = new Row
                {
                    ...
                    CommonRate = ParseFloat(Cell(cells, map, "commonrate"), "CommonRate", lineNumber),
                    UnCommonRate = ParseFloat(Cell(cells, map, "uncommonrate"), "UnCommonRate", lineNumber),
                    RareRate = ParseFloat(Cell(cells, map, "rarerate"), "RareRate", lineNumber),
                    EpicRate = ParseFloat(Cell(cells, map, "epicrate"), "EpicRate", lineNumber),
                    LegendaryRate = ParseFloat(Cell(cells, map, "legendaryrate"), "LegendaryRate", lineNumber),
                    MythicRate = ParseFloat(Cell(cells, map, "mythicrate"), "MythicRate", lineNumber),
        ...
        private static void Validate(Row row, int line)
        {
            ...
            ValidateRate(row.CommonRate, "CommonRate", line);
            ValidateRate(row.UnCommonRate, "UnCommonRate", line);
            ValidateRate(row.RareRate, "RareRate", line);
            ValidateRate(row.EpicRate, "EpicRate", line);
            ValidateRate(row.LegendaryRate, "LegendaryRate", line);
            ValidateRate(row.MythicRate, "MythicRate", line);

            float total = row.CommonRate +
                          row.UnCommonRate +
                          row.RareRate +
                          row.EpicRate +
                          row.LegendaryRate +
            ...
            {
                throw new FormatException(
                    $"Dòng {line}: Tổng tỷ lệ phải bằng 100, hiện tại là {total.ToString("0.####", CultureInfo.InvariantCulture)}.");
            }
        ...
        private static void AddLevel(SerializedProperty list, Row row)
        {
            ...
            SetFloatCandidate(item, row.CommonRate, "CommonRate", "commonRate");
            SetFloatCandidate(item, row.UnCommonRate,
                "UnCommonRate", "UncommonRate", "unCommonRate", "uncommonRate");
            SetFloatCandidate(item, row.RareRate, "RareRate", "rareRate");
            SetFloatCandidate(item, row.EpicRate, "EpicRate", "epicRate");
            SetFloatCandidate(item, row.LegendaryRate, "LegendaryRate", "legendaryRate");
            SetFloatCandidate(item, row.MythicRate, "MythicRate", "mythicRate");
