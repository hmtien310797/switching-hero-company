...
namespace Battle
{
    public class Boss01SkillController : BaseSkillController
    {
        [SerializeField] MonsterBossController monsterBoss;

        public override void DoAttack(Action endAct)
        {
            monsterBoss?.IsLookRight();
        ...
        {
            SkaFx.transform.position = new Vector3(monsterBoss?.Target.transform.position.x ?? .5f, 0.5f, monsterBoss?.Target.transform.position.z ?? 0f);
