...
namespace Battle.Dungeon
{
    public sealed class DungeonKillAllMode : IDungeonMode
    {
        ...
        private int stage;
        private DungeonKillAllDto dungeonKillAllDto;

        public UniTask InitializeAsync(DungeonModeContext context)
        {
            this.context = context;
            dungeonName = context.RuntimeData.TempDungeonName;
            stage = context.RuntimeData.Stage;
            spawnedCount = 0;
            killedCount = 0;
            aliveCount = 0;
            nextBatchDelay = 0f;
            active = false;
            dungeonKillAllDto = new DungeonKillAllDto(dungeonName, stage, killedCount, context.RuntimeData.TotalEnemyCount);
            GameEventManager.Trigger(GameEvents.OnKillAllDungeonInit, dungeonKillAllDto);
        ...
        public void NotifyEnemyDead(EnemyActor enemy)
        {
            ...
            dungeonKillAllDto.SetData(killedCount);
            GameEventManager.Trigger(GameEvents.OnKillAllDungeonEnemyCountChanged, killedCount, context.RuntimeData.TotalEnemyCount);
    ...
    public struct DungeonKillAllDto
    {
        ...
        public DungeonKillAllDto(string dungeonName, int stage, int killedCount, int totalEnemyCount)
        {
        ...
        public void SetData(int killCount)
        {
